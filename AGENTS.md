# Codex Impact Lab — Project Memory

Context for every Codex session in this repo.

## The event
Mumbai | Codex Impact Lab (2026-06-27). One-day hackathon. Build a Codex-powered
...
- **Model:** `Codex-opus-4-8` (set in `backend/app/config.py` — change there once).
...
- Keep the backend thin; put one Codex call behind one endpoint.
- Use the latest, most capable Codex model by default.
